...
function validateLockerInput(lockerId, capacity, currentCount, tLevels, tColumns, tDepth) {
  ...
  router.route('/addLocker').post(async (req, res) => {
    ...
    const checkSql = 'SELECT * FROM LockerM WHERE lockerId = ?';
    db.query(checkSql, [sanitizedInput.lockerId], (err, results) => {
      ...
      const insertSql = 'INSERT INTO LockerM (lockerId, capacity, currentCount, tLevels, tColumns, tDepth, lastUpdate) VALUES (?, ?, ?, ?, ?, ?, ?)';
      db.query(insertSql, [
  ...
  router.route('/deleteLocker/:lockerId').delete(async (req, res) => {
    ...
    const sql = 'DELETE FROM LockerM WHERE lockerId = ?';
  ...
  router.route('/LockerUpdate/:lockerId').put(async (req, res) => {
    ...
    db.query(checkSql, [lockerId], (checkErr, checkResult) => {
      ...
      const updateSql = 'UPDATE LockerM SET capacity = ?, currentCount = ?, tLevels = ?, tColumns = ?, tDepth = ?, lastUpdate = ? WHERE lockerId = ?';
      db.query(updateSql, [sanitizedInput.capacity, sanitizedInput.currentCount, sanitizedInput.tLevels, sanitizedInput.tColumns, sanitizedInput.tDepth, req.body.lastUpdate, lockerId], (updateErr, updateResult) => {
        if (updateErr) {
