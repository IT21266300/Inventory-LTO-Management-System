...
const LockerTable = ({ result, loading, error }) => {
  ...
  const handleUpdate = () => {
    navigate('/headofficeLUpdate', { state: { data: passValue } });
  };
  ...
  const handleDelete = async () => {
    ...
    setOpenAlert(false);
    try {
      axios.delete(`/api/lockerH/deleteLocker/${passValue.lockerId}`);
        ...
        <Box>
          <ActionButton handleClick={handleClick}
           params={params}
            open={open} />
        <Link
          to={`/lockerH/${params.row.lockerId}`}
